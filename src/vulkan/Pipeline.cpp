...
Pipeline::Pipeline(const CreatePipelineArgs& a_args) {
	...
	GetDevice([&](auto& a_device) {
		...
		vk::UniqueShaderModule vertModule = a_device.createShaderModuleUnique(vertInfo);
		vk::UniqueShaderModule fragModule = a_device.createShaderModuleUnique(fragInfo);
		...
		m_descriptorSetLayout = a_device.createDescriptorSetLayout(dslInfo);
		...
		m_pipeLineLayout = a_device.createPipelineLayout(layoutInfo);
		...
		m_pipeline = a_device.createGraphicsPipeline(vk::PipelineCache{}, pipeInfo);
	});
}
	...
	if(m_pipeline) {
		ExecuteNextFrame(
		    [pipeline = m_pipeline, pipeLineLayout = m_pipeLineLayout, descriptorSetLayout = m_descriptorSetLayout]() {
			    GetDevice([&](auto& a_device) {
				    a_device.destroyPipeline(pipeline);
				    a_device.destroyPipelineLayout(pipeLineLayout);
				    a_device.destroyDescriptorSetLayout(descriptorSetLayout);
			    });
		    });
	}
